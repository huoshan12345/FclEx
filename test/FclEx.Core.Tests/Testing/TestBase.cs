...
public abstract class TestBase<T> : TestBase
{
    ...
    {
        var cases = _EnumerableTestData().Distinct().ToArray();
        return cases.ToTheoryData();

        static IEnumerable<(EnumerableType, int SetLength, int EnumerableLength, int NumberOfMatchingElements, int NumberOfDuplicateElements)> _EnumerableTestData()
        {
            foreach (var collectionSizeArray in ValidCollectionSizes())
            {
                foreach (var enumerableType in Enum.GetValues<EnumerableType>())
                {
