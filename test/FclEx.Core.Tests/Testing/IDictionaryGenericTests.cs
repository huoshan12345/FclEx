...
public abstract class IDictionaryGenericTests<TKey, TValue> : ICollectionGenericTests<KeyValuePair<TKey, TValue>>
{
    ...
    {
        var collection = GenericIDictionaryFactory();
    ...
    {
        var dictionary = GenericIDictionaryFactory(count);
        var existingKey = GetNewKey(dictionary);
        dictionary.Add(existingKey, CreateTValue(5342));
        var newValue = CreateTValue(1234);
    ...
    public void ICollection_Generic_Remove_ValidPresentKeyWithDifferentValue(int count)
    {
        if (!IsReadOnly)
        {
            var dictionary = GenericIDictionaryFactory(count);
            var missingKey = GetNewKey(dictionary);
            var present = CreateTValue(324);
            var missing = CreateTValue(5612);
