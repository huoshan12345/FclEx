...
public class List_Generic_Tests_Insert
{
    internal class Driver<T>
    {
        ...
        {
            var list = new List<T>(constructIEnumerable(itemsX));
        ...
        {
            var list = new List<T>(constructIEnumerable(items));
            int[] bad = [items.Length + 1, items.Length + 2, int.MaxValue, -1, -2, int.MinValue];
            for (var i = 0; i < bad.Length; i++)
            {
        ...
        {
            var list = new List<T>(items);
            var range = useSlice ? list.Slice(index, count) : list.GetRange(index, count);
        ...
        {
            var list = new List<T>(items);
            var range = list[index..(index + count)];
        ...
        {
            var list = new List<T>(items);
            var range = useSlice ? list[index..(index + count)] : list.GetRange(index, count);
    ...
    public static void InsertTests()
    {
        ...
        IntDriver.NonGenericIListBasicInsert([], 1, 0, 3);
    ...
    public static void InsertRangeTests()
    {
        ...
        {
            IntDriver.InsertRangeIEnumerable([], intArr1, 0, 1, collectionGenerator);
        ...
        {
            StringDriver.InsertRangeIEnumerable([], stringArr1, 0, 1, collectionGenerator);
    ...
    public static void TrueForAll_ListSizeCanBeChanged()
    {
        ...
        var expectedList = new List<int> { 1, 2, 3, 2, 3, 4, 3, 4, 4 };

        var result = list.TrueForAll(i =>
        {
            if (i < 4)
            {
                list.Add(i + 1);
