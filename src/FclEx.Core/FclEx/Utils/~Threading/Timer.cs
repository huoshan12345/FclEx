...
/// <summary>
/// Invokes a stateful callback on a <see cref="System.Threading.Timer"/> schedule.
/// </summary>
/// <remarks>
...
/// </remarks>
public sealed class Timer<T> : IDisposable, IAsyncDisposable
{
    ...
    public Timer(TimerCallback<T> callback, T state, TimeSpan dueTime, TimeSpan period)
    {
        ...
        _lifetime = new(new System.Threading.Timer(s => callback(s.CastTo<T>()!), state, dueTime, period));
    }
