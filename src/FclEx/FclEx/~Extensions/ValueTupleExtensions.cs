...
namespace FclEx
{
    public static class ValueTupleExtensions
    {
        ...
        public static string? FirstValid(this IEnumerable<string?> values, int? count = null)
        {
            var q = values;
            if (count.HasValue)
                q = q.Take(count.Value);
            return q.FirstOrDefault(m => m.IsValid());
        }

        public static string? FirstValid(this (string?, string?) tuple)
        {
            const int count = 2;
            ...
            arr[0] = tuple.Item1;
            arr[1] = tuple.Item2;
            return arr.FirstValid(count);
        }

        public static string? FirstValid(this (string?, string?, string?) tuple)
        {
            const int count = 3;
            using var disposable = ObjectPoolHelper.GetArrayPool<string?>().GetAsDisposable(count);
            var arr = disposable.Value;
        ...
        public static string? FirstValid(this (string?, string?, string?, string?) tuple)
        {
            const int count = 4;
            using var disposable = ObjectPoolHelper.GetArrayPool<string?>().GetAsDisposable(count);
            var arr = disposable.Value;
