...
{
    private readonly List<(object? value, Type type)> _args = new();
    ...
    private static IList<ArgumentMatchResult> GetMatches(IReadOnlyList<ParameterInfo> paras, IReadOnlyList<ArgumentInfo> args)
    {
        if (paras.Count == 0)
        {
            return new[] { new ArgumentMatchResult(), };
        }
    ...
    private class ArgumentMatchInfo
    {
        public ArgumentMatchInfo(ArgumentInfo argument, ArgumentMatchType matchType, int paraIndex, IEnumerable<int> remainArgIndexes)
        {
            Argument = argument;
            MatchType = matchType;
            RemainArgIndexes = new HashSet<int>(remainArgIndexes);
            ParaIndex = paraIndex;
        }
