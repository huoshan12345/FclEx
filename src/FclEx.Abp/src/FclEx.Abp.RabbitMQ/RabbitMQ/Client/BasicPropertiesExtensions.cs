...
public static class Extensions
{
    ...
    public static T? Get<T>(this IBasicProperties prop, string key, T? defaultValue = default)
    {
        ...
        var obj = prop.Headers?.GetOrDefault(key);
    ...
    public static int IncreErrorTimes(this IBasicProperties prop)
    {
        if (prop == null) throw new ArgumentNullException(nameof(prop));
        prop.Headers ??= new Dictionary<string, object>();
    ...
    {
        return prop?.Headers?.GetOrDefault(FclExAbpRabbitMqConstants.HeaderOfDelayMilli)?.CastTo<int>() ?? 0;
    }
