...
51. [Resolved] `src/FclEx.Http/FclEx/Http/~Services/HttpClientServiceBase.cs`: manual redirects now drop `Authorization`, explicit `Cookie`, and target-specific headers such as `Host`, `Origin`, and `Referer`.
...
53. [Resolved] `src/FclEx.Http/FclEx/Http/~Auth/ClientCredentialsTokenProvider.cs`: `GetTokenAsync` creates an `HttpClient` before checking whether a valid access token is already cached, and `GetDiscoveryAsync` creates one before the second `_discovery` check inside the lock. Cache hits can still allocate/dispose clients or trigger factory side effects. Move client creation until after cache/discovery checks that can return immediately.

54. [Resolved] `src/FclEx.Http/FclEx/Http/~Services/HttpServiceExtensions.cs`: `BatchDownloadAsync` returns a lazy `IEnumerable<Task<...>>` and reuses the same `DownloadOptions.Content` instance for every request. Since sending disposes `request.Content`, multiple downloads or repeated enumeration can reuse a disposed content object and start duplicate downloads. Materialize the tasks or expose an async collection API, and use a content factory/buffer per request when a body is needed.

55. [Resolved] `src/FclEx.Http/FclEx/Http/~Core/HttpRequestExtensions.cs`: `Dump(this HttpRequest, IEnumerable<Cookie>)` obtains a cached `StringBuilder` but never disposes the cache handle, unlike the other `StringBuilderHelper.GetCached()` call sites. Wrap it in `using var` so the builder is returned to the cache.

56. [Resolved] `src/FclEx.Http/FclEx/Http/~Core/HttpRequestExtensions.cs` and `src/FclEx.Http/FclEx/Http/~Services/HttpClientServiceBase.cs`: trace logging calls `currentRequest.Dump(this)`, and `Dump(this HttpRequest, IHttpService)` calls `service.GetCookies(request.GetUri())`. For relative request URIs that rely on `HttpClient.BaseAddress`, cookie lookup may throw before the request is sent. Make dumping tolerate relative URIs or pass the resolved/base URI into the dump path.

57. [Documented] `src/FclEx.Http/FclEx/Http/~Extensions/HttpRequestMessageExtensions.cs`: public `SetNotSend` mutates the private send-status field of `HttpRequestMessage` through reflection or `UnsafeAccessor`, and it is not used internally. This is brittle across runtime versions and encourages reusing sent messages. Remove it, make it internal test-only, or replace it with explicit request cloning.
...
60. [Resolved] `src/FclEx.Http/FclEx/Http/~Core/DownloadOptions.cs` and `src/FclEx.Http/FclEx/Http/~Services/HttpServiceExtensions.cs`: timeout naming in the download APIs is ambiguous. `DownloadOptions.ConnectTimeout` is applied to `HttpRequest.ReadHeadersTimeout`, while the simpler `DownloadAsync(..., TimeSpan? timeout)` overload applies its timeout to `ReadBufferTimeout`. Rename the option to `ReadHeadersTimeout` or split the API into clearly named total/header/body timeout options.

61. [Resolved] `src/FclEx.Http/FclEx/Http/~Services/HttpServiceExtensions.cs`: `BatchDownloadAsync` buffers `options.Content` once and clones it for each request, but the buffered content created from a non-buffered `options.Content` is not disposed after all downloads finish. Wrap the buffered content lifetime in `try/finally`, while preserving ownership of a caller-supplied `BufferedContent` if that is intended.
...
65. [Resolved] `src/FclEx.Http/FclEx/RegexesExtensions.cs`: `Regexes.CharSet` exposes the mutable backing `Regex[]`. External callers can mutate the array and change global charset detection behavior for the process. Return a read-only collection, an immutable array, or a defensive copy.

66. [Resolved] `src/FclEx.Http/FclEx/Http/~Actions/JsonActionContext.cs`, `src/FclEx.Http/FclEx/Http/~Actions/HtmlActionContext.cs`, and `src/FclEx.Http/FclEx/Http/~Actions/XmlActionContext.cs`: the context property/constructor parameter is still named `Path` even though the public action APIs now use `JsonPath`, `HtmlSelector`, and `XPath`. Rename these context members to match their selector language and reduce the leftover ambiguity from the earlier `*ResultPath` naming.

67. [Resolved] `src/FclEx.Http/AngleSharp/Dom/ElementExtensions.cs`: `QueryHref` returns `OperationResult`, but invalid href values or invalid base/relative URI combinations can throw `UriFormatException` directly from `new Uri(...)`. Catch URI creation failures and return an error result, or document that only missing attributes are represented as operation errors.

68. [Resolved] `src/FclEx.Http/FclEx/Http/~Core/HttpRequestExtensions.Header.cs`: `AcceptCn` is an unclear public name for setting `Accept-Language: zh-CN,zh;q=0.8`. Rename it to something more natural and behavior-focused, such as `AcceptChinese`, `AcceptSimplifiedChinese`, or `AcceptChineseLanguage`.

69. [Tested] `src/FclEx.Http/FclEx/Http/~Options/HttpClientOptionsEqualityComparer.cs`: `Equals` compares `BaseAddress` with `StringComparison.OrdinalIgnoreCase`, but `GetHashCode` uses the original `AbsoluteUri` string. Two options that compare equal can produce different hash codes, which can break `LfuCache<HttpClientOptions, IServiceProvider>` lookups. Normalize the URI hash with the same comparison semantics, for example through `StringComparer.OrdinalIgnoreCase`.

70. [Tested] `src/FclEx.Http/FclEx/Http/~Utils/WebProxyEqualityComparer.cs`: proxy credentials are compared and hashed by object identity. Two proxies created from the same URI with the same embedded user/password can be treated as different because each call creates a new `NetworkCredential`. Compare credential values for supported credential types, especially `NetworkCredential`, so `HttpClientService` provider caching behaves predictably.

71. [Documented] `src/FclEx.Http/FclEx/Http/~Extensions/HttpMessageInvokerExtensions.cs` and `src/FclEx.Http/FclEx/Http/~Extensions/HttpClientExtensions.cs`: `GetHandler` and `GetPrimaryHandler` expose public APIs that read the private `_handler` field through reflection. This is brittle across runtime versions and may be trimming/AOT-hostile, similar to `SetNotSend`. Consider making these test-only/internal, documenting the runtime dependency clearly, or replacing the use cases with explicit handler injection.

72. [Tested] `src/FclEx.Http/FclEx/Http/~HttpContents/CompressedContent.cs`: `SerializeToStreamAsync` obtains the inner content stream with `Content.ReadAsStreamAsync(Token)` but does not dispose that stream after copying. Some `HttpContent` implementations return disposable stream resources. Wrap the returned stream in `using`/`await using` while keeping the existing ownership rule that disposing `CompressedContent` disposes the inner content.

73. [Tested] `src/FclEx.Http/FclEx/Http/~Handlers/DefaultOptionsHandler.cs`: `SendAsync` casts `request.Options` to `IDictionary<string, object?>` before applying defaults. That depends on the concrete runtime implementation of `HttpRequestOptions`; if the interface shape changes, the handler can fail before sending. Prefer the public `HttpRequestOptions.Set` API for each option key.

74. [Resolved] `src/FclEx.Http/FclEx/Http/~Actions/IXmlAction.cs`: this file contains `IXmlAction<T>`, `IXmlAction`, `DefaultXmlAction`, `XmlAction<T>`, and `XmlAction`, while JSON/HTML action defaults and base classes are split into their own files. Split the XML default implementation and base classes into separate files to match the rest of the action folder and improve discoverability.

75. [Tested] `src/FclEx.Http/AngleSharp/Dom/ElementExtensions.cs`: `GetFormData` now handles common successful controls, but it still misses a few HTML form edge cases. Controls inside the first `legend` of a disabled `fieldset` should remain successful, and options inside a disabled `optgroup` should not be submitted. Add these cases if the helper is intended to model browser form submission closely.

76. [Resolved] `src/FclEx.Http/FclEx/Http/~Actions/DefaultHtmlFileAction.cs`: `GetResponseAsync` uses reflection through `PropertyInfos` to set `HttpResponse.ResponseString` and `HttpResponse.StatusCode`, even though the method is in the same assembly and those setters are internal. Assign the properties directly or add a small factory method on `HttpResponse`; this removes unnecessary reflection and makes the intent clearer.
