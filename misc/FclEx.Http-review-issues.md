...
52. [Open] `src/FclEx.Http/FclEx/Http/~Services/HttpClientServiceBase.cs` and `src/FclEx.Http/FclEx/Http/~Helpers/HtmlHelper.cs`: charset auto-detection from HTML meta tags calls `Encoding.GetEncoding(charSet)` directly, so `IgnoreInvalidCharSet = true` does not protect callers when the detected meta charset is invalid. The regex in `GetMetaCharSet` is also too narrow for common forms such as `<meta charset=utf-8>`. Route detected charsets through the same validation path as header/fallback charsets, and consider parsing or broadening the meta charset extractor.

53. [Open] `src/FclEx.Http/FclEx/Http/~Auth/ClientCredentialsTokenProvider.cs`: `GetTokenAsync` creates an `HttpClient` before checking whether a valid access token is already cached, and `GetDiscoveryAsync` creates one before the second `_discovery` check inside the lock. Cache hits can still allocate/dispose clients or trigger factory side effects. Move client creation until after cache/discovery checks that can return immediately.

54. [Open] `src/FclEx.Http/FclEx/Http/~Services/HttpServiceExtensions.cs`: `BatchDownloadAsync` returns a lazy `IEnumerable<Task<...>>` and reuses the same `DownloadOptions.Content` instance for every request. Since sending disposes `request.Content`, multiple downloads or repeated enumeration can reuse a disposed content object and start duplicate downloads. Materialize the tasks or expose an async collection API, and use a content factory/buffer per request when a body is needed.

55. [Open] `src/FclEx.Http/FclEx/Http/~Core/HttpRequestExtensions.cs`: `Dump(this HttpRequest, IEnumerable<Cookie>)` obtains a cached `StringBuilder` but never disposes the cache handle, unlike the other `StringBuilderHelper.GetCached()` call sites. Wrap it in `using var` so the builder is returned to the cache.

56. [Open] `src/FclEx.Http/FclEx/Http/~Core/HttpRequestExtensions.cs` and `src/FclEx.Http/FclEx/Http/~Services/HttpClientServiceBase.cs`: trace logging calls `currentRequest.Dump(this)`, and `Dump(this HttpRequest, IHttpService)` calls `service.GetCookies(request.GetUri())`. For relative request URIs that rely on `HttpClient.BaseAddress`, cookie lookup may throw before the request is sent. Make dumping tolerate relative URIs or pass the resolved/base URI into the dump path.

57. [Open] `src/FclEx.Http/FclEx/Http/~Extensions/HttpRequestMessageExtensions.cs`: public `SetNotSend` mutates the private send-status field of `HttpRequestMessage` through reflection or `UnsafeAccessor`, and it is not used internally. This is brittle across runtime versions and encourages reusing sent messages. Remove it, make it internal test-only, or replace it with explicit request cloning.

58. [Open] `src/FclEx.Http/FclEx/Web/~Core/UserClientLogger.cs`: `UserClientLogger` reflects the internal `Microsoft.Extensions.Logging.FormattedLogValues` type and private fields. This can break when `Microsoft.Extensions.Logging` changes its implementation. Prefer a public logging abstraction, a scope/property wrapper, or accept prefix formatting without reconstructing internal state.

59. [Open] `src/FclEx.Http/FclEx/RegexesExtensions.cs`: `Regexes.MetaRefresh` and `Regexes.MetaRefreshUrl` remain public even though meta refresh extraction now uses AngleSharp. These stale regex helpers duplicate old behavior and may keep callers on the less robust path. Remove them or clearly mark them as legacy if they must remain.
