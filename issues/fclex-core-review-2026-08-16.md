...
211. **[P1] 名称为“Bytes”的 marshal API 输出的是悬挂/进程内指针，而不是可安全传输的字节。**
...
212. **[P0] `UnsafeHelper` 的按字节读写允许复制含 GC 引用的结构并越过缓冲区边界。**
...
213. **[P0] `UnsafeHelper.GetValue<T>(IntPtr)` 可把任意地址伪造为托管引用。**
...
214. **[P1] 自定义 delegate 工厂在不可回收动态程序集持续定义类型。**
...
215. **[P1] 公开修改 `Exception` 私有字段的 API 不具备跨运行时契约。**
...
216. **[P2] `RetryHelper` 在异常筛选器中执行用户重试谓词，会吞掉谓词自身异常。**
...
217. **[P2] `ExpiringLazy<T>` 用墙上时钟实现有效期，且允许首次访问时溢出。**
...
218. **[P1] batch consumer 把正在重试的内部数组作为 `IReadOnlyList<T>` 通知给外部。**
...
219. **[P2] consumer 的观察事件在消费循环线程同步执行，监听器实际控制吞吐和停机。**
...
220. **[P2] `NameValues` 构建把任意值 `ToString()` 成协议数据，却没有格式化策略。**
...
221. **[P1] `WhenDefault` 对值类型永远可能判断失败。**
   - 位置：`Utils/~Collections/INameValuesBuilder.cs` 的 `DefaultNameValuesBuilder.Build`。
...
222. **[P2] `NameValueOmitOption.Never` 与可组合 flags 的模型矛盾。**
   - 位置：`Utils/~Collections/NameValueOmitOption.cs`。
...
223. **[P3] `ConsoleTable.AddRow` 以通用 `Exception` 报告可预期的参数/状态错误。**
   - 位置：`Utils/~ConsoleTable/ConsoleTable.cs`。
...
228. **[P1] `DictionaryExtensions.Add` 的两个泛型重载对常见“字典到集合”形状形成歧义。**
   - 位置：`Extensions/~System/~Collections/~Generic/DictionaryExtensions.cs`。
...
229. **[P2] `CrossJoin` 假定输入可重复枚举，却接受任意 `IEnumerable`。**
   - 位置：`Extensions/~System/~Collections/~Generic/EnumerableExtensions.cs`。
...
231. **[P1] `AnyContainsAny`、`AnyContainsAll` 等方法会重复消费两个 `IEnumerable<string>`。**
   - 位置：`Extensions/~System/~Collections/~Generic/EnumerableExtensions.String.cs`。
   - 说明：嵌套 `Any` 对一次性序列会在第一次外层迭代耗尽 `values`，结果取决于枚举器状态而非字符串关系。
   - 建议：物化较小/需要重用的一侧，或改用集合参数；不能把“可重复枚举”作为未声明前提。

232. **[P2] 串行 `ToOperationIOPairsSerially` 在最后一项后仍等待 interval。**
   - 位置：`Extensions/~System/~Collections/~Generic/EnumerableExtensions.IOPair.cs`。
...
234. **[P2] `AddWeeks` 先执行 int 乘法，极端输入会悄然溢出。**
   - 位置：`Extensions/~System/DateTimeExtensions.cs`。
...
235. **[P2] `AsyncEventHandlerExtensions.GetInvocationList<T>` 用 `Unsafe.As` 谎称返回了 `T[]`。**
   - 位置：`Extensions/~System/AsyncEventHandlerExtensions.cs`。
...
236. **[P2] `ReadOnlyListExtensions.TryGet` 的 `[NotNullWhen(true)]` 承诺不成立。**
   - 位置：`Extensions/~System/~Collections/~Generic/ReadOnlyListExtensions.cs`。
...
237. **[P2] 带 group index 的 `RegexExtensions.TryMatch` 没有遵守 Try 模式。**
   - 位置：`Extensions/~System/~Text/~RegularExpressions/RegexExtensions.cs`。
...
238. **[P2] span split 枚举器在非法 `Current` 状态返回 default 或陈旧数据。**
   - 位置：`Extensions/~System/ReadOnlySpanExtensions.cs` 的 split enumerator。
...
239. **[P3] `ToBytes(ReadOnlySpan<bool>)` 隐藏了 bit packing 的位序。**
   - 位置：`Extensions/~System/ReadOnlySpanExtensions.cs`。
...
240. **[P2] `BuildType`/`IsDebug`/`IsRelease` 把 JIT 标志误报为编译配置。**
   - 位置：`Extensions/~System/~Reflection/AssemblyExtensions.cs`。
...
241. **[P3] `MemberInfoExtensions.IsDefined<T>` 缺少 `where T : Attribute`。**
   - 位置：`Extensions/~System/~Reflection/MemberInfoExtensions.cs`。
...
242. **[P2] `MethodInfoExtensions.GetSignature` 生成的字符串并不唯一。**
   - 位置：`Extensions/~System/~Reflection/MethodInfoExtensions.cs`。
...
243. **[P2] `TypeExtensions.CreateObject` 把 null 的 params 数组变成“一个 null 参数”。**
   - 位置：`Extensions/~System/TypeExtensions.cs`。
...
245. **[P2] `ReflectionHelper.AccessorAccessesField` 逐字节扫描 IL，会把操作数误识别为 opcode。**
   - 位置：`Helpers/ReflectionHelper.cs`。
...
246. **[P2] `TimerLifetime.BeginDispose` 在首个释放路径抛异常时可能让并发释放者自旋不止。**
   - 位置：`Utils/~Threading/TimerLifetime.cs`。
...
247. **[P2] `CancellationTokenSource.TryCancel` 无差别吞掉回调异常和对象生命周期错误。**
   - 位置：`Extensions/~System/~Threading/CancellationTokenSourceExtensions.cs`。
...
249. **[P2] `StreamExtensions.ReadAllTextAsync` 默认关闭由调用方提供的 stream。**
   - 位置：`Extensions/~System/~IO/StreamExtensions.cs`。
...
251. **[P3] `SemaphoreSlimExtensions.IsEmpty` 将瞬时快照包装成状态判断。**
   - 位置：`Extensions/~System/~Threading/SemaphoreSlimExtensions.cs`。
...
1. 先处理 210、212、213 与 218：它们涉及进程稳定性、内存安全和并发工作项所有权。
2. 接着定下 202–207、214–222 的长期 API 与生命周期模型，特别是序列化、超时和名称值构建。
3. 再完成 223–251 的签名、命名、异常类型和边界行为统一，并为每个已确认的行为补充针对性测试。
