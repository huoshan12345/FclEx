...
19. **[P2] `ISqlAdapter.EnableIdentityInsertAsync<T>` 把泛型实体、命令和 scope 生命周期混在方言接口中。**
...
26. **[P1] 表达式版 `GetQuotedColumnName` 对带 `[Column]` 的属性必然按错误名字查找。**
...
27. **[P1] `[Table(..., Schema=...)]` 的 schema 被忽略。**
...
28. **[P1] `[NotMapped]` 和 `DatabaseGeneratedOption.Computed` 仍会进入 INSERT。**
...
29. **[P1] 元数据把所有 public property 当作可插入列，包括静态、索引器、只读和导航属性。**
...
30. **[P1] `[Column(TypeName=...)]` 被当成 provider enum 名称解析，误用了标准属性语义。**
